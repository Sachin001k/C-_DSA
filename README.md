# C++ Recursion

<!DOCTYPE html>
<html>
  <head>
    <!--this is head-->

    <title>myFirstwebsite</title>
    <style>
      h1 {
        color: red;
      }
      h2 {
        color: blue;
      }
      table {
        border-collapse: collapse;
      }

      td {
        border: 1px solid purple;
      }
      th {
        border: 1px solid black;
      }
    </style>
  </head>

  <body style="color: purple">
    <!--body of the website-->
    <p>
      <!--this is paragraph-->
    </p>
    <h1>heading</h1>
    <h2>heading 2</h2>
    <h1>new heading 1</h1>
    <!--there are 6 heading tags-->
    this is body
    <br />
    <p style="color: red">
      Lorem ipsum dolor sit amet consectetur adipisicing elit. Nisi enim, quae
      non illum exercitationem inventore similique vero molestias, quaerat,
      explicabo eius? Facere nihil ducimus illo neque nisi distinctio saepe
      vitae velit minima perspiciatis esse ut, qui voluptatum asperiores ullam
      vel, sed, consectetur aliquam assumenda fugiat autem. Minima fugit facere
      repellendus incidunt vel cupiditate exercitationem aperiam, a praesentium,
      sapiente blanditiis quos distinctio suscipit earum maiores nobis, quisquam
      qui sunt unde aspernatur ad ducimus accusamus porro dolorem. Consequuntur
      quae beatae ratione reprehenderit sunt exercitationem asperiores
      reiciendis distinctio placeat accusamus! Reprehenderit beatae ipsam
      dolores quo porro asperiores minus fuga dolorem. Non, velit quibusdam!
    </p>

    <br />
    <p>
      Lorem, ipsum dolor sit amet consectetur adipisicing elit. Aut fugiat quas
      iusto odio veritatis nobis maiores numquam voluptate labore dolore.
      Quaerat fugit in provident repellat beatae perferendis quam temporibus
      magni.
    </p>
    <hr />
    <img
      style="height: 200px"
      src="
    https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQQuJe8MJRmcAR7oICqyUG66VwtGSZMkhDkbQ&usqp=CAU"
      alt=""
    />
    <img style="height: 200px" src="./e5.webp" alt="" />
    <hr />

    <a href="https://leetcode.com/problemset/all/"> Leetcode</a>
    <hr />
    <a href="./dummyfile.html">go to dummmy page</a>
    <hr />
    <br />
    <img src="./dummyfile.html" alt="" />

    <video width="300px" height="500px" controls>
      <source src="./video.mp4" alt="video/mp4" />
    </video>

    <video width="300px" height="500px" controls>
      <source src="./v2.mp4" alt="v2/mp4" />
    </video>

    <br/>
    <video width="900px" height="1600px" controls>
      <source src="https://youtu.be/dObM0GvvD48"
    </video>

    <a href="./2nd_file.html">Link to next file</a>
    <a style="color: blueviolet" href="https://google.com">
      <img
        src="data:image/jpeg;base64,"
        alt=""
      />
    </a>
    <br />
    <!--ordered list-->
    <ol>
      <li>first</li>
      <li>second</li>
      <li>something else</li>
    </ol>
    <br />
    <!--unordered list-->
    <ul>
      <li>one</li>
      <li>two</li>
      <li>something more about unordered list</li>
    </ul>
    <!--creating a table-->
    <table>
      <tr>
        <th>Name</th>
        <th>Roll No</th>
        <th>Branch</th>
      </tr>
      <tr>
        <td>Sachin</td>
        <td>190732</td>
        <td>Aerospace</td>
      </tr>
      <tr>
        <td>Aman</td>
        <td>190002</td>
        <td>Computer Science</td>
      </tr>
    </table>
    <!--time table-->
    <hr />
    <br />
    <table>
      <!--1st row-->
      <tr>
        <th rowspan="6">Time Table</th>
        <th>Monday</th>
        <th>Tuesday</th>
        <th>wednessday</th>
      </tr>
      <!--2nd row-->
      <tr>
        <td>DSA</td>
        <td>2nd Lecture</td>
        <td>web dev</td>
      </tr>
      <!--3rd row-->
      <tr>
        <td>DSA</td>
        <td>maths</td>
        <td>web dev</td>
      </tr>
      <!--4th row-->
      <tr>
        <td>DSA</td>
        <td>maths</td>
        <td>web dev</td>
      </tr>
      <!--break time-->
      <tr>
        <td colspan="3">break time</td>
      </tr>
      <!--6th row-->
      <tr>
        <td>DSA</td>
        <td>maths</td>
        <td>web dev</td>
      </tr>
    </table>
    <a href="./forms.html">forms</a>
  </body>
</html>
